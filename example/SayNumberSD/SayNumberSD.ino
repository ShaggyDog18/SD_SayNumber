...
void setup() {
  ...
  //----Set volume----
  Say.volume(26);  //Set volume value (0~30).
  ...
  for( int32_t i = -3; i < 1023; i++ ) {
    ...
    if( i > 1 && i <= 3 ) errorStatus *= Say.sayAny( SAY_KM );
    else if( i > 3 && i <= 6 ) errorStatus *= Say.sayAny( SAY_METER );
    else if( i > 6 && i <= 9 ) errorStatus *= Say.sayAny( SAY_MILE );
    Serial.println( errorStatus ? "~OK~" : "~Error!~" );
